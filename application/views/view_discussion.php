...
<img src="<?php echo get_gravatar($app->email, 164, 'identicon'); ?>" style="float:left; padding:5px;" />
	<h1><?php echo anchor("app-profile/$app->appnameid", $app->appname); ?></h1>
...
<h2><?php echo $discussion->title; ?></h2>
		...
		<?php
			$author = $this->db->get_where("users", array("id" => $discussion->userid));
			$author = $author->row(); ?>
		<cite><?php echo anchor("user-profile/" . $author->id, $author->name); ?> <br /><em><?php echo date('M d, Y - g:i:s A', strtotime($discussion->createdate)); ?></em></cite>
		</blockquote>
		<?php $comments = $this->db->get_where("discussion", array("parentid" => $discussion->id));
		...
		<div style="float:left; width:450px;">
			<blockquote>
				...
				<?php
					...
					$author = $author->row(); ?>
				<cite><?php echo anchor("user-profile/" . $author->id, $author->name); ?> <br /><em><?php echo date('M d, Y - g:i:s A', strtotime($comment->createdate)); ?></em></cite>
			</blockquote>
		</div>
		</div>
		<?php $i++;endforeach; ?>
